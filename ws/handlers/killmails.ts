...
function generateRoutingKeys(killmail: any): string[] {
  ...
  const securityStatus = killmail.solarSystem?.securityStatus ?? killmail.securityStatus
  ...
  if (regionId !== undefined && regionId !== null) {
    if (regionId >= WSPACE_REGION_MIN && regionId <= WSPACE_REGION_MAX) {
      keys.push('w-space')
      keys.push('wspace')
    } else if (regionId >= ABYSSAL_REGION_MIN && regionId < ABYSSAL_REGION_MAX) {
      keys.push('abyssal')
  ...
  if (shipGroupId) {
    ...
    if (SHIP_GROUPS.battlecruisers.includes(shipGroupId)) keys.push('battlecruisers')
    ...
    if (SHIP_GROUPS.supercarriers.includes(shipGroupId)) keys.push('supercarriers')
