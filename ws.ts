...
const CLEANUP_INTERVAL = parseInt(process.env.WS_CLEANUP_INTERVAL || '60000', 10) // 1 minute
...
function broadcastKillmail(killmail: any): void {
  let sentCount = 0
  const messageType = killmailMessageHandler.getMessageType?.(killmail) || 'killmail'
  const logId = killmailMessageHandler.getLogIdentifier?.(killmail) || 'unknown'

  for (const [ws, clientData] of clients.entries()) {
    if (ws.readyState === 1) { // OPEN
      const shouldSend = killmailMessageHandler.shouldSendToClient?.(killmail, clientData) ?? false
  ...
  if (sentCount > 0) {
    logger.debug(`Broadcasted ${messageType} ${logId} to ${sentCount}/${clients.size} clients`)
  }
}
...
function handleMessage(ws: any, message: string): void {
  try {
    ...
    switch (data.type) {
      ...
      case 'subscribe':
        if (Array.isArray(data.topics)) {
          const validTopics = data.topics.filter(topic =>
            killmailMessageHandler.isValidTopic?.(topic) ?? false
          )
          const invalidTopics = data.topics.filter(topic =>
            !(killmailMessageHandler.isValidTopic?.(topic) ?? false)
          )
      ...
      case 'unsubscribe':
        if (Array.isArray(data.topics)) {
          clientData.topics = clientData.topics.filter(t => !data.topics!.includes(t))
          ws.send(JSON.stringify({
            type: 'unsubscribed',
            topics: data.topics,
          }))
  ...
  for (const [ws, clientData] of clients.entries()) {
    if (clientData.lastPing) {
      ...
      if (timeSincePing > PING_TIMEOUT) {
        ...
        if (!hasRecentPong) {
          logger.info(`Removing unresponsive client (${timeSincePing}ms since ping)`)
...
function startPingPongMonitoring(): void {
  ...
  logger.info(`Started ping/pong monitoring (interval: ${PING_INTERVAL}ms, timeout: ${PING_TIMEOUT}ms)`)
}
