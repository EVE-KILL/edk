...
export default defineEventHandler(async (event: H3Event) => {
  try {
    ...
    const [stats, dashboardData, [killmailsData, totalKillmails]] = await Promise.all([
      track('alliance:losses:fetch_stats', 'application', async () => {
      ...
      getDashboardData(allianceId, 'alliance', 'losses', userFilters),
      track('alliance:losses:fetch_killmails', 'application', async () => {
