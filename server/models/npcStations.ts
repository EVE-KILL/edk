...
export class NPCStationQueries {
  static async getStation(stationId: number) {
    const result = await database.query(
      `SELECT * FROM npcStations FINAL WHERE stationId = {stationId:UInt32} LIMIT 1`,
      { stationId }
    )
  ...
  static async getStationsBySystem(solarSystemId: number) {
    return await database.query(
      `SELECT * FROM npcStations FINAL WHERE solarSystemId = {solarSystemId:UInt32} ORDER BY stationId`,
      { solarSystemId }
    )
  }
  ...
  static async getStationName(stationId: number): Promise<string | null> {
    const result = await database.query(
      `SELECT name FROM npcStations FINAL WHERE stationId = {stationId:UInt32} LIMIT 1`,
      { stationId }
    )
  ...
  static async getStationsByOperation(operationId: number) {
    return await database.query(
      `SELECT * FROM npcStations FINAL WHERE operationId = {operationId:UInt32} ORDER BY stationId`,
      { operationId }
    )
  }
}
