 ...
 *     description: Returns a paginated list of agent-types from the Static Data Export.
 *     tags:
 *       - SDE - NPCs
 *     parameters:
 *       - name: page
...
export default defineEventHandler(async (event) => {
  ...
  const [items, totalResult] = await Promise.all([
    database.query<AgentType>(
      `SELECT * FROM agenttypes ORDER BY "agentTypeId" LIMIT :limit OFFSET :offset`,
