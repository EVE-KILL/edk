...
export default {
  ...
  async action(options: any) {
    if (!options.table) {
      logger.error(
        'Usage: bun run cli sde:inspect --table <name> [--rows <count>]'
      );
...
function displayInspectionResults(result: InspectionResult): void {
  ...
  // Display schema
  logger.info('Schema:');
  const sortedFields = Array.from(result.schema.entries()).sort((a, b) =>
  ...
  if (result.sampleRows.length > 0) {
    logger.info(
      `Sample Data (${chalk.blue(result.sampleRows.length.toString())} rows):`
    );
  ...
  for (const [field, types] of result.fields.entries()) {
    if (types.size > 1) {
      logger.debug(
        `${field.padEnd(35)} ${chalk.magenta(`[${Array.from(types).join(', ')}]`)}`
      );
    }
  }
}
