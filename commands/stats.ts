...
class StatsDashboard {
  ...
  async run(options: any): Promise<void> {
    ...
    if (snapshot) {
      logger.info(chalk.blue.bold('📊 EDK Stats Dashboard - Snapshot\n'));
  ...
  private async updateTUI(): Promise<void> {
    try {
      ...
      this.widgets.header.setContent(
        `{center}📊 EDK Stats Dashboard - ${timestamp}{/center}`
      );
      ...
      this.widgets.dbStorage.setContent(
        `  {yellow-fg}Total Database:{/}\n` +
          ...
          dbStats.tables
            .filter((t) =>
              ['characters', 'corporations', 'alliances'].includes(t.name)
            )
      ...
      if (this.widgets.header) {
        this.widgets.header.setContent(
          '{center}{red-fg}⚠ Error fetching stats{/}{/center}'
        );
      }
    }
  }
  ...
  private formatQueueStats(stats: QueueStats): string {
    ...
    content +=
      '  {bold}Queue{/}            {bold}Active{/}            {bold}Waiting{/}           {bold}Prioritized{/}       {bold}Completed{/}         {bold}Failed{/}\n';
    content += '  ' + '─'.repeat(110) + '\n';
    ...
    for (const queueType of queueOrder) {
      if (stats[queueType]) {
        ...
        const formatWithDelta = (
          ...
          const coloredNum =
            current > 0 ? `{${color}-fg}${numStr}{/}` : `{gray-fg}${numStr}{/}`;
        ...
        const prioritized = formatWithDelta(
          s.prioritized,
          prev?.prioritized,
          'magenta'
        );
        const completed = formatWithDelta(
          s.completed,
          prev?.completed,
          'green'
        );
  ...
  private async displayStats(): Promise<void> {
    try {
      ...
      if (this.running) {
        console.clear();
        logger.info(chalk.blue.bold('📊 EDK Stats Dashboard - Live Mode'));
        logger.info(chalk.dim(`Updated: ${new Date().toLocaleString()}\n`));
      }
  ...
  private displayDatabaseStats(stats: DatabaseStats): void {
    ...
    logger.info(chalk.white('  Storage:'));
    logger.info(
      `    Total Size:           ${chalk.yellow(stats.databaseSize)}`
    );
    logger.info(
      `    Killmail Tables:      ${chalk.yellow(stats.killmailsRelatedSize)} ${chalk.dim('(km+attackers+items)')}`
    );
    logger.info(chalk.white('  Counts (Top Tables):'));
    for (const table of stats.tables.slice(0, 6)) {
      const name = table.name.charAt(0).toUpperCase() + table.name.slice(1);
      logger.info(
        `    ${name.padEnd(16)} ${chalk.green(table.count.toLocaleString().padStart(10))} ${chalk.dim(`(${table.size})`)}`
      );
    }
    logger.info(chalk.white('  Activity:'));
    logger.info(
      `    Last 24h:             ${chalk.magenta(stats.recentKillmails24h.toLocaleString())} killmails`
    );
    logger.info(
      `    Active Connections:   ${chalk.blue(stats.activeConnections.toString())}`
    );
    logger.info('');
  }

  private displayQueueStats(stats: QueueStats): void {
    logger.info(chalk.cyan.bold('━━━ QUEUES ━━━'));
    ...
    for (const queueType of queueOrder) {
      if (stats[queueType]) {
        ...
        logger.info(chalk.white(`  ${queueType.toUpperCase()}:`));
        logger.info(
          `    Active:       ${statusColor(s.active.toLocaleString())}`
        );
        logger.info(
          `    Waiting:      ${statusColor(s.waiting.toLocaleString())}`
        );
        logger.info(
          `    Prioritized:  ${chalk.magenta(s.prioritized.toLocaleString())}`
        );
        logger.info(
          `    Completed:    ${chalk.green(s.completed.toLocaleString())}`
        );
        logger.info(
          `    Failed:       ${s.failed > 0 ? chalk.red(s.failed.toLocaleString()) : chalk.gray('0')}`
        );
      }
    }
    logger.info('');
  ...
  private displayRedisStats(stats: RedisStats): void {
    logger.info(chalk.cyan.bold('━━━ REDIS ━━━'));

    if (stats.connected) {
      logger.info(`  Status:      ${chalk.green('✓ Connected')}`);
      logger.info(`  Memory:      ${chalk.yellow(stats.usedMemory)}`);
      logger.info(`  Keys:        ${chalk.blue(stats.keys.toLocaleString())}`);
      logger.info(
        `  Uptime:      ${chalk.white(this.formatUptime(stats.uptime))}`
      );
    } else {
      logger.info(`  Status:      ${chalk.red('✗ Disconnected')}`);
    }
    logger.info('');
  }

  private displayWebSocketStats(stats: WebSocketStats): void {
    logger.info(chalk.cyan.bold('━━━ WEBSOCKET ━━━'));

    if (stats.available) {
      logger.info(`  Status:      ${chalk.green('✓ Running')}`);
      logger.info(
        `  Clients:     ${chalk.blue(stats.connectedClients.toString())}`
      );
    } else {
      logger.info(`  Status:      ${chalk.gray('Not running')}`);
    }
    logger.info('');
  }
  ...
  private displayPrometheusMetrics(): void {
    try {
      ...
      logger.info(chalk.cyan.bold('━━━ PROMETHEUS METRICS ━━━'));

      if (metrics) {
        logger.info(chalk.white('  HTTP Requests: Available'));
      }

      if (dbMetrics) {
        logger.info(chalk.white('  DB Metrics: Available'));
      }

      logger.info(chalk.gray(`  Full metrics: GET /metrics`));
      logger.info('');
    } catch {
