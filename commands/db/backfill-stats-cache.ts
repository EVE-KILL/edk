...
async function backfillAllTimeStats(entityType: 'character' | 'corporation' | 'alliance') {
  ...
  logger.success(`✓ Backfilled ${count} ${entityType}s (all-time) in ${duration}ms`);
...
async function backfillTimeBucketStats(
  ...
  logger.info(`Found ${entities.length} active ${entityType}s in last ${days} days`);
  ...
  const duration = Date.now() - startTime;
  logger.success(`✓ Backfilled ${processed} ${entityType}s (${days}d) in ${(duration / 1000).toFixed(1)}s`);
...
async function backfillStatsCache() {
  ...
  try {
    ...
    logger.info('');
    logger.success(`✓ Phase 1 complete: ${totalEntities.toLocaleString()} entities`);
    logger.info('');
    ...
    logger.info('1. Verify data: SELECT COUNT(*), "entityType" FROM entity_stats_cache GROUP BY "entityType"');
    logger.info('2. Triggers are now active and will maintain stats in real-time');
    logger.info('3. Schedule daily cleanup job (see cronjobs/cleanup-stats-cache.ts)');
    logger.info('');
...
export default () => ({
  description: 'Backfill entity stats cache from existing killmails data (one-time operation, takes 1-2 hours)',
  action,
});
