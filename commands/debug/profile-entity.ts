...
import { getEntityStats, type EntityStats } from '../../server/models/entityStats'
import { getEntityKillmails, countEntityKillmails } from '../../server/models/killlist'
import { getMostValuableKillsByCharacter, getMostValuableKillsByCorporation, getMostValuableKillsByAlliance } from '../../server/models/mostValuableKills'
...
export const description = 'Profile entity page queries (character/corporation/alliance)'

export const options = [
  {
    flags: '--type <type>',
    description: 'Entity type: character, corporation, or alliance (default: character)'
  },
  {
    flags: '--id <id>',
...
export async function action(options: CommandOptions) {
  const entityType = (options.type?.toLowerCase() || 'character') as 'character' | 'corporation' | 'alliance'
  ...
  try {
    if (warmupIterations > 0) {
      logger.info(`Running ${warmupIterations} warm-up iteration${warmupIterations === 1 ? '' : 's'} (results discarded)`)
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      for (const measurement of measurementsByDuration) {
        ...
        console.log(`  - ${measurement.label.padEnd(32)} ${details.join(' | ')}`)
      }
...
function createTasks(params: {
  ...
  return [
    ...
    },
    {
      ...
      collectInfo: (result: EntityStats | null) => ({ note: result ? `kills=${result.kills}, losses=${result.losses}` : 'none' })
    },
    {
      key: 'entity:killmails:list',
      label: `Killmails page ${page} (kills+losses)`,
      category: 'killmails',
      run: async () => await getEntityKillmails(entityId, entityType, 'all', page, perPage),
      collectInfo: (result) => ({ rowCount: Array.isArray(result) ? result.length : undefined })
    },
    {
  ...
  const measurements = await Promise.all(
    tasks.map(async (task) => {
      ...
      try {
        ...
        const info = task.collectInfo ? task.collectInfo(result) : collectDefaultInfo(result)
        return {
          key: task.key,
          label: task.label,
...
function aggregateByCategory(measurements: TaskMeasurement[]): Map<EntityCategory, number> {
  const totals = new Map<EntityCategory, number>()
  for (const measurement of measurements) {
    totals.set(measurement.category, (totals.get(measurement.category) ?? 0) + measurement.durationMs)
  }
  return totals
}
...
function printTaskTable(stats: Map<string, any>): void {
  ...
  for (const [, stat] of stats) {
    ...
    rows.push({
      ...
      notes: stat.note ? `${stat.rowCount ? `rows=${stat.rowCount} | ` : ''}${stat.note}` : stat.rowCount ? `rows=${stat.rowCount}` : ''
    })
  }
  ...
  for (const row of rows) {
    ...
    console.log(`  ${row.label.padEnd(32)} avg=${formatMs(row.avg)} | min=${formatMs(row.min)} | max=${formatMs(row.max)}${notes}`)
  }
}
...
function printCategoryTable(stats: Map<EntityCategory, number[]>): void {
  ...
  for (const row of rows) {
    console.log(`  ${row.category.padEnd(20)} avg=${formatMs(row.avg)} | min=${formatMs(row.min)} | max=${formatMs(row.max)}`)
  }
}
