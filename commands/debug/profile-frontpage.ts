...
import { countFilteredKills, getFilteredKillsWithNames } from '../../server/models/killlist'
...
export const description = 'Profile frontpage data queries (killlist, top boxes, most valuable kills)'
...
function createTasks(params: { page: number; perPage: number }): PerfTask[] {
  ...
  const topBoxEntityTypes: Array<['character' | 'corporation' | 'alliance' | 'ship' | 'system' | 'region', string]> = [
...
function aggregateByCategory(measurements: TaskMeasurement[]): Map<FrontpageCategory, number> {
...
function printCategoryTable(categoryStats: Map<FrontpageCategory, number[]>): void {
  ...
  const header = 'Category'.padEnd(14) + 'Avg (ms)'.padStart(10) + 'Min'.padStart(10) + 'Max'.padStart(10)
  console.log(header)
  console.log('-'.repeat(header.length))
