...
export default {
  ...
  options: [
    {
      flags: '--delay <ms>',
      description:
        'Initial delay between API fetches in milliseconds (default: 1000)',
      defaultValue: '1000',
    },
    {
  ...
  action: async (options: { delay?: string; startDate?: string }) => {
    ...
    logger.info(
      `Starting zKillboard backfill, starting from ${options.startDate || 'today'}...`
    );
    ...
    if (!historyResponse.ok) {
      logger.error(
        `Failed to fetch history totals from zKillboard. Status: ${historyResponse.status}`
      );
      return;
    }
    ...
    const totalKillmails = dateEntries.reduce(
    ...
    logger.info(
      `Found ${dateEntries.length} days to process, with a total of ${totalKillmails} killmails.`
    );
      ...
      while (!success) {
        ...
        if (response.ok) {
          ...
          if (killmailData.length > 0) {
            ...
            const newKillmails = killmailData.filter(
              (k) => !existingIds.has(k.killmailId)
            );
          ...
          logger.warn(
            `Rate limited. Increasing delay to ${delay}ms and retrying...`
          );
          await new Promise((resolve) => setTimeout(resolve, delay));
        } else {
          logger.error(
            `Failed to fetch killmails for ${dateStr}. Status: ${response.status}. Skipping this date.`
          );
          break;
        }
      }
