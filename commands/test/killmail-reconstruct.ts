...
export default {
  description:
    'Test killmail reconstruction - Fetches from DB and outputs in ESI format',
  options: [
    {
  ...
  async action(options: { id?: string }) {
    if (!options.id) {
      logger.error(
        'Usage: bun run cli test:killmail-reconstruct --id <killmail_id>'
      );
      logger.error(
        'Example: bun run cli test:killmail-reconstruct --id 130756307'
      );
      return;
    }
    ...
    logger.info(
      `Fetching killmail ${chalk.cyan(id.toString())} from database...`
    );
    ...
    try {
      ...
      if (!killmail) {
        logger.error(
          `Killmail ${chalk.red(id.toString())} not found in database`
        );
        return;
      }
