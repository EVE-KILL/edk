...
export const schedule = '0 2 * * *'; // Daily at 2 AM
...
export async function handler() {
  ...
  try {
    ...
    logger.success(`✓ Stats cache cleanup completed in ${(duration / 1000).toFixed(1)}s`);
...
async function cleanupTimeBucket(
  ...
  const duration = Date.now() - startTime;
  logger.info(`  ✓ Cleaned ${processed} ${entityType}s in ${(duration / 1000).toFixed(1)}s`);
}
