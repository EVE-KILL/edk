...
export class Controller extends WebController {
  ...
  override async handle(): Promise<Response> {
    ...
    // Fetch ship group LOSS statistics for the last 30 days
    ...
    const data = {
      ...
      baseUrl: "/entities",
      entityList,
      // Ship group statistics (losses only)
      shipGroupStats,
      shipGroupColumns,
      stats: {
      ...
      pagination: {
        ...
        nextPageUrl: hasNextPage
          ? `/entities/losses?page=${currentPage + 1}`
          : null,
      ...
      // Filter config for WebSocket killlist updates (only losses where entities were victims)
      filterConfig: {
        type: 'losses',
    ...
    return await this.renderPage(
      "pages/entities-losses",
      "All Entities - Losses",
      "Loss history for all followed entities",
      data
    );
  }
}
