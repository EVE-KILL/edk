...
import {
  createStreamingHtmlResponse,
  splitHtmlForStreaming,
  createStreamingJsonResponse,
...
export abstract class BaseController {
   ...
   * Useful for large pages where you want to send content progressively
   *
   * @param template - Template name to render
   * @param data - Data to pass to template
   * @param layout - Layout template
   * @returns Streaming response
   */
  protected async streamingHtmlResponse(
    template: string,
    data: TemplateData = {},
    layout: string = "main"
  ): Promise<Response> {
    try {
      ...
      // Split HTML into streamable chunks
      const { head, body, footer } = splitHtmlForStreaming(html);

      // Create streaming response with current headers
      ...
      (async () => {
        try {
          // Send head first (contains CSS, meta tags - important for fast render)
          await write(head);
      ...
      console.error("Streaming template error:", error);
      return this.errorResponse("Template rendering failed", 500);
    }
  }
   ...
   * @returns Streaming JSON array response
   */
  protected async streamingJsonResponse<T>(items: T[]): Promise<Response> {
    const headers = new Headers();
    Object.entries(this.headers).forEach(([key, value]) => {
      headers.set(key, value);
    });
    ...
    (async () => {
      try {
        ...
        for (let i = 0; i < items.length; i++) {
          await writeArrayItem(items[i], i === 0);
        }

        await writeArrayEnd();
        await end();
      } catch (error) {
        console.error("Streaming JSON error:", error);
        await end();
      }
    })();
  ...
  /**
   * Create a streaming paginated JSON response
   ...
   */
  protected async streamingPaginatedResponse<T>(
    ...
    const response = await streamPaginatedResponse(fetchPage, totalPages, headers);
