...
(function() {
  ...
  if (!window.__EDK_WS_URL) {
    console.error('[DEV-RELOAD] ❌ WebSocket URL not configured, skipping hot reload');
    return;
  }
  ...
  function connect() {
    ...
    try {
      ...
      ws.addEventListener('message', function(event) {
        try {
          ...
          if (message.type === 'direct' && message.data) {
            ...
            if (dataType === 'server-reload' || dataType === 'template-reload') {
              const filename = message.data.filename;
              
              if (dataType === 'server-reload') {
                console.log('[DEV-RELOAD] 🔄 Server restarted - reloading page...');
              } else if (dataType === 'template-reload') {
                console.log(`[DEV-RELOAD] 📝 Template changed: ${filename || 'unknown'} - reloading page...`);
              }
              
              // Save scroll position before reload
              sessionStorage.setItem('dev-reload-scroll', window.scrollY.toString());
      ...
      ws.addEventListener('close', function() {
        console.log('[DEV-RELOAD] ⚠️  Connection closed - attempting reconnect...');
        ws = null;
        attemptReconnect();
      });
  ...
  function attemptReconnect() {
    ...
    reconnectAttempts++;
    console.log(`[DEV-RELOAD] Reconnecting in ${RECONNECT_DELAY}ms (attempt ${reconnectAttempts}/${MAX_RECONNECT_ATTEMPTS})...`);
